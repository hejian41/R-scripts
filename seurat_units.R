...
AddAVscore2 <- function (object, artery.genes = artery.genes, venous.genes = venous.genes, gene.max = 10) 
{
  artery.genes = toupper(c("Efnb2","Dll4","Hey1","Gja4","Unc5b"))
  venous.genes = toupper(c("Ephb4","Nr2f2","Nrp2","Aplnr","Flt4"))
  enrich.name <- "A_V"
  genes.list <- list(artery.Score = artery.genes, venous.Score = venous.genes)
  object@data <- apply(object@data, 2, function(x){gene.max*x/max(x)})
  object.av <- AddModuleScore(object = object, genes.list = genes.list, 
                              enrich.name = enrich.name, ctrl.size = min(vapply(X = genes.list, 
                                                                                FUN = length, FUN.VALUE = numeric(1))))
  av.columns <- grep(pattern = enrich.name, x = colnames(x = object.av@meta.data))
  av.scores <- object.av@meta.data[, av.columns]
  rm(object.av)
  gc(verbose = FALSE)
  assignments <- apply(X = av.scores, MARGIN = 1, FUN = function(scores, first = "Artery", second = "Venous") {
    return(c(first, second)[which(x = scores == max(scores))])
  })
  av.scores <- merge(x = av.scores, y = data.frame(assignments), 
                     by = 0)
  colnames(x = av.scores) <- c("rownames", "artery.score", "venous.score", 
                               "A_V state")
  rownames(x = av.scores) <- av.scores$rownames
  av.scores <- av.scores[, c("artery.score", "venous.score", "A_V state")]
  object@meta.data <- data.frame(object@meta.data, av.scores)
  return(object)
}
...
myCCPlot2 <- function(pbmc, group.by = NULL, text.size = 3, cols.use = palette(),...){
  group.colors <- sort(unique(if(is.null(group.by)) pbmc@ident else pbmc@meta.data[[group.by]]))
  require(dplyr)
  require(ggplot2)
  ccData <- pbmc@meta.data 
  ccx_min <- floor(min(ccData$S.Score))
  ccy_min <- floor(min(ccData$G2M.Score))
  ccx_max <- ceiling(max(ccData$S.Score))
  ccy_max <- ceiling(max(ccData$G2M.Score))
  p1 <- ggplot(ccData) + geom_point(mapping = aes_string("S.Score", "G2M.Score", color = group.by), size = 3) + 
    xlab(label = "S phase score") + ylab("G2/M phase score") + ggtitle("Cell Cycle Analysis") +
    scale_color_manual(values = cols.use[as.factor(group.colors)]) +
    geom_linerange(mapping = aes_(x = 0, ymin = ccy_min, ymax = 0)) + 
    geom_segment(mapping = aes_(x = ccx_min, y = 0, xend = 0, yend = 0)) + 
    geom_segment(mapping = aes_(x = 0, y = 0, xend = ccx_max, yend = ccy_max)) +
    geom_text(mapping = aes_(ccx_min/2, ccy_min/1.5, label = quote("G1"), hjust = 1.1)) +
    geom_text(mapping = aes_(ccx_max/2, ccy_max/1/5, label = quote("S"), hjust = 1.1)) +
    geom_text(mapping = aes_(ccx_min/2, ccy_max/2, label = quote("G2M"), hjust = 1.1)) +
    theme_bw() + theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.5))
  
  ccData <- pbmc@meta.data %>% count_(vars = c(group.by, "Phase")) %>% group_by_(group.by) %>% dplyr::arrange(desc(Phase)) %>%
    dplyr::mutate(pct = n/sum(n),
           ypos = cumsum(pct) - 0.5*pct)
  p2 <- ggplot(ccData) + 
    geom_bar(mapping = aes_string(x = group.by, y = "pct", fill = "Phase"), stat = "identity", width = 0.8) + 
    geom_text(mapping = aes_string(x = group.by, y= "ypos",label = quote(paste0(ccData$n,", ",sprintf("%1.1f", 100*ccData$pct),"%"))), size = text.size) +
    ggtitle(label = "Cell cycle Distribution of Cells") + 
    theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 45, hjust =1, vjust = 1))
  #  pheatmap::pheatmap(expr, cluster_cols = F, cluster_rows = F, annotation_col = annot, border_color = NA)
...
myAVPlot <- function(pbmc, group.by = NULL, continuous = F, gene.max = 10, use.lim = T, pt.size = 2, text.size = 3, cols.use = palette(), aggregate.fun = mean, ...){
  ...
  if(continuous) p1 + scale_color_gradientn(colours = cols.use)
    ...
    p2 <- ggplot(av_ggData,aes(x = Venous_average, y = Artery_average)) + 
      ...
      geom_label_repel(aes(label = Average)) + 
...
myAVPlot2 <- function(pbmc, group.by = NULL, continuous = F, gene.max = 10, use.lim = T, pt.size = 2, text.size = 3, cols.use = palette(), aggregate.fun = mean, ...){
  # atery venous score
  require(ggplot2)
  require(gridExtra)
  pbmc@meta.data$ident <- pbmc@ident[rownames(pbmc@meta.data)]
  ...
  p1 <- ggplot(data = pbmc@meta.data, mapping = aes_string(x = "venous.score", y = "artery.score", color = group.by)) + 
    geom_point(size = pt.size) + 
    coord_fixed(ratio = 1)
  if(use.lim) p1 <- p1 + xlim(-4,gene.max) + ylim(-4,gene.max)
  if(continuous) p1 + scale_color_gradientn(colours = cols.use)
  else{
    p1 <- p1 + scale_color_manual(values = cols.use)
    ...
    p2 <- ggplot(av_ggData,aes(x = Venous_average, y = Artery_average)) + 
      ...
      scale_color_manual(values = cols.use) + 
      coord_fixed(ratio = 1) + guides(color = guide_legend(title = group.by))
	if(use.lim) p2 <- p2 + xlim(-2,gene.max/2) + ylim(-2,gene.max/2)
    grid.arrange(p1,p2, nrow = 1)
  } 
}
